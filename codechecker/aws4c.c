...
static size_t header ( void * ptr, size_t size, size_t nmemb, void * stream )
{
  ...
  if (!strncmp ( (char *) ptr, "HTTP/1.1", 8 ))
    {
      b->result = strdup ( (char *) ptr + 9 );
      __chomp(b->result);
      b->code   = atoi ( (char *) ptr + 9 );
    }
  else if ( !strncmp ( (char *) ptr, "ETag: ", 6 ))
    {
      b->eTag = strdup ( (char *) ptr + 6 );
      __chomp(b->eTag);
    }
  else if ( !strncmp ( (char *) ptr, "Last-Modified: ", 14 ))
    {
      b->lastMod = strdup ( (char *) ptr + 15 );
      __chomp(b->lastMod);
    }
  else if ( !strncmp ( (char *) ptr, "Content-Length: ", 15 ))
    {
      b->contentLen = atoi ( (char *) ptr + 16 );
    }
